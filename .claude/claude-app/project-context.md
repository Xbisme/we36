...
> Last updated: 2026-07-07 (#001–#010 all merged; **#010 Profile & Follow ✅ MERGED** into `main` via **PR #10** (`f2e28b1`) — 57/57 tasks (US1–US6: my/other profile, optimistic follow/unfollow, followers/following lists, viewer-side private accounts, edit profile + avatar). Next: **#011 Saved Collections**.)
...
- **Now**: **Spec #010 Profile & Follow ✅ MERGED** into `main` via **PR #10** (`f2e28b1`) — **57/57 tasks done** (US1–US6). The **identity + social-graph** surface: my profile (US1), other-user profile + optimistic follow (US2), followers/following lists (US3), viewer-side private accounts (US4), edit profile + change photo (US5), inclusive/adaptive (US6). Contract-driven on the shipped backend **B#010**; app still runs DI `environment: 'fake'` for hermetic tests.
  - **Built + verified**: `lib/features/profile/` (`my_profile`/`profile`/`follow_list`/`edit_profile` cubits [4-state], usecases [profile/follow/follow-list/edit], `my_profile_page`/`user_profile_page`/`follow_list_page`/`edit_profile_page`, widgets [`profile_header`/`profile_stats`/`profile_tab_bar`/`profile_grid`/`follow_button`/`account_row_tile`/`private_gate`/`avatar_picker_sheet`]) + `lib/core/data/profile/` (`ProfileView` + `AccountRow` freezed, `ProfileRepository` real + `FakeProfileRepository`, `profile_remote_data_source`, in-memory `relationship_store`). Reuses shipped models: `MeProfile` (#003), `User` (#002), `ViewerRelationship` (#009), feed `Post`/grid (#004), #007 media-upload pipeline (avatar). Posts + Tagged tabs (both cursor-paginated); optimistic follow (instant) / **confirm-on-unfollow**; canonical relationship + own-following-count update; private = counts shown, lists+grid gated; avatar pick→crop→upload writes `avatarMediaId` (completes the avatar #003 deferred). **No drift schema change** (relationship state = in-memory store; drift stays v7). No new pub dependency.
  - **Post-merge gate fixed this session (2026-07-07, on `main`) → 509 pass / 0 fail**: PR #10 merged with 4 pre-existing failures unrelated to #010 (stories & post-detail untouched). Cleaned up: **2× #005 stories time-bomb** (`stories_repository_impl_test` seeded `2026-07-02` vs a real-clock 24h TTL → own "Your story" filtered as expired; **fixed** by freezing `OwnStoryStore`'s injectable clock in `setUp`, no prod change) + **2× #006 comment-list goldens** (0.06% sub-pixel drift → regenerated). Gate green.
  - **▶ Next**: start **#011 Saved Collections** (`git checkout -b 011-collections` + `/speckit.specify`). Depends on #006 (save action) + #010; Screen 24. Scope: save posts into named collections, collections grid, add/remove, the profile "saved" tab (deferred out of #010).
...
| 011 | Saved Collections | 🟡 **Next** | `011-collections` |
