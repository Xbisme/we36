...
> Last updated: 2026-07-10 (#001–#013 all merged to `main`. **#013 Notifications & Push ✅ MERGED** via PR #13 (merge `e19097a`) — 56/56 tasks; the Activity surface (Screen 29) + FCM/APNs push + deep-linking + **second live wiring** of the #002 `RealtimeClient` (`notification.new`); drift v9→v10; Firebase `we36-ed5a4` config wired for the dev flavor. B#013 source-verified. **Next: #014 Settings, Privacy & Safety.**)
...
- **Now**: **#001–#013 all merged to `main`** (green gate). **▶ Next: Spec #014 Settings, Privacy & Safety** — start with `git checkout -b 014-settings-privacy` then `/speckit.specify`. Scope (Screens 30–32): account settings · **private account** · **close friends** list · **blocking** + **report** (enforced in UI immediately) · activity status · two-factor entry · language (EN/VI/system) · theme (light/dark/system — palette fixed) · download-your-data entry · about/version. **Carry-over**: approving incoming **follow-requests** was deferred here (from #010/#013). New packages: `shared_preferences` (already in tree since #003), `package_info_plus`, `in_app_review`. Depends on most prior specs — **unblocked**. App still runs DI `environment: 'fake'` (zero-network).
  - **#013 ✅ MERGED into `main`** via PR #13 (merge `e19097a`; 56/56 tasks). Activity feed (Screen 29, New / This week / Earlier) + follow-back · **FCM/APNs push** + contextual permission + deep-link · **second live wiring** of the #002 `RealtimeClient` (`notification.new`). drift **v9→v10** (`Notifications` table). Firebase `we36-ed5a4` config wired for the **dev flavor** (Android `google-services.json` + iOS plist in the Runner target); background handler + push-tap deep-linking wired in `bootstrap.dart`. B#013 **source-verified**. New deps `firebase_core` · `firebase_messaging` · `flutter_local_notifications`. iOS deploy target 13→15. **Remaining for on-device push** (→ #015/cutover): APNs auth key in Firebase console · prod config for `app.we36` · run real DI env against a live backend to exercise `POST /devices` · on-device receipt smoke.
...
- **Active blockers**: none. `main`'s gate is green after #013 merged (PR #13).
