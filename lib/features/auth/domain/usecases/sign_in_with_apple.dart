...
/// Apple sign-in: obtain an identity token on-device, exchange it with the
/// backend (create-or-link), then start the session. A user cancel →
/// `oauthCancelled` (handled silently by the UI, spec FR-021).
@injectable
class SignInWithApple {
  const SignInWithApple(this._source, this._auth, this._session);
  ...
  Future<Result<MeProfile>> call() async {
    final idToken = await _source.appleIdToken();
    if (idToken == null) {
